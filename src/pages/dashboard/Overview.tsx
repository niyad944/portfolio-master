  ...
  return (
    <div className="p-4 sm:p-8 lg:p-12 max-w-7xl mx-auto">
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-10 sm:mb-14 perspective-1200"
      >
        {statCards.map((stat, index) => (
          <motion.div key={index} variants={itemVariants}>
            <Tilt3DCard maxTilt={6} scale={1.02}>
              <Link
                to={stat.link}
                className="stats-card group cursor-pointer block shadow-3d-hover"
              >
      ...
      <ScrollReveal3D depth={50}>
        <div className="mb-10 sm:mb-14">
          ...
          <motion.div
            ...
            initial="hidden"
            animate="visible"
            className="grid sm:grid-cols-2 gap-4 sm:gap-5 perspective-1200"
          >
            {quickActions.map((action, index) => (
              <motion.div key={index} variants={itemVariants}>
                <Tilt3DCard maxTilt={4} scale={1.01}>
                  <Link
                    to={action.link}
                    className="glass-card-hover rounded-2xl p-5 sm:p-7 flex items-start gap-4 sm:gap-5 group block shadow-3d-hover"
                  >
      ...
      {stats.certificates === 0 && stats.projects === 0 && (
        <ScrollReveal3D depth={60}>
          <div className="glass-card rounded-2xl p-6 sm:p-10 gradient-border-animated">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-7">
              <Tilt3DCard maxTilt={8} scale={1.03} className="shrink-0">
                <div className="w-18 h-18 sm:w-20 sm:h-20 rounded-2xl bg-accent/10 border border-accent/25 flex items-center justify-center shadow-glow float-3d">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-4">
                  <Link to="/dashboard/about">
                    <Button className="bg-accent hover:bg-accent/90 text-accent-foreground btn-glow btn-3d-lift shadow-glow rounded-xl min-h-[48px] px-6 transition-all duration-500 hover:shadow-bloom">
                      Complete Profile
                      <ArrowRight className="ml-2 w-5 h-5" />
                    </Button>
                  </Link>
                  <Link to="/dashboard/certificates">
                    <Button variant="outline" className="border-white/10 hover:bg-white/[0.05] rounded-xl min-h-[48px] px-6 btn-3d-lift transition-all duration-400">Upload Certificate</Button>
                  </Link>
                </div>
              </div>
            </div>
