...
export const AchievementsSection = ({ achievements }: { achievements: Achievement[] }) => {
  ...
  return (
    <Section id="achievements">
      <SectionHeading eyebrow="Recognition" title={<>Achievements <span className="text-slate-500">& milestones.</span></>} />
            ...
            <TiltCard max={4}>
              <GlassCard className="p-6 h-full group relative overflow-hidden">
                ...
                <div className="relative">
                  <div className="h-11 w-11 rounded-2xl grid place-items-center bg-gradient-to-br from-[hsl(var(--pf-cyan))]/20 to-[hsl(var(--pf-violet))]/20 border border-slate-200 mb-4">
                    <Trophy className="h-5 w-5 text-[hsl(var(--pf-cyan))]" />
                  </div>
                  <h3 className="font-display text-lg font-semibold text-slate-900">{a.title}</h3>
                  {a.issuer && <p className="mt-1 text-sm text-slate-500">{a.issuer}</p>}
                  {a.date && (
                  ...
                  {a.description && (
                    <p className="mt-3 text-sm text-slate-600 leading-relaxed line-clamp-4">{a.description}</p>
                  )}
                </div>
              </GlassCard>
            </TiltCard>
