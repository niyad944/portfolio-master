              ...
              <Reveal key={e.id} delay={i * 0.06}>
                <div className={`relative grid md:grid-cols-2 gap-6 ${right ? "" : "md:[&>*:first-child]:col-start-2"}`}>
                  <div className={`pl-12 md:pl-0 ${right ? "md:pr-12 md:text-right" : "md:pl-12"}`}>
                    <div className="relative rounded-2xl border border-slate-200 bg-white backdrop-blur-xl p-6 hover:border-slate-300 transition-colors">
                      ...
                      <div className={`flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-slate-500 mb-3 ${right ? "md:justify-end" : ""}`}>
                        <Calendar className="h-3 w-3" />
                        {e.start_date} — {e.end_date || "Present"}
                      </div>
                      <h3 className="font-display text-xl font-semibold text-slate-900">{e.degree}</h3>
                      <p className="mt-1 text-slate-600">{e.institution}</p>
                      {e.field_of_study && <p className="mt-1 text-sm text-slate-500">{e.field_of_study}</p>}
                      {e.grade && (
                        <div className={`mt-4 flex ${right ? "md:justify-end" : ""}`}>
                          <span className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-[hsl(var(--pf-cyan))]/20 to-[hsl(var(--pf-violet))]/20 border border-slate-200 px-3.5 py-1.5 text-sm font-semibold text-slate-900 shadow-[0_0_20px_hsl(var(--pf-cyan)/0.15)]">
